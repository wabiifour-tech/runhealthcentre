...
async function sendWithBrevo(to: string, code: string, name: string, apiKey: string): Promise<{ success: boolean; error?: string }> {
  ...
  const SENDER_NAME = process.env.EMAIL_SENDER_NAME || "Redeemer's University Health Centre (RUHC)"
  ...
  try {
    const response = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        subject: "Your Verification Code - Redeemer's University Health Centre (RUHC)",
        htmlContent: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #1e40af, #10b981); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
              <h1 style="color: white; margin: 0;">Redeemer's University Health Centre (RUHC)</h1>
            ...
            <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e2e8f0; border-top: none;">
              ...
              <p style="color: #475569; font-size: 16px;">Thank you for registering at Redeemer's University Health Centre (RUHC). Please use the following verification code to complete your registration:</p>
              ...
              <p style="color: #94a3b8; font-size: 12px; text-align: center;">
                Redeemer's University Health Centre (RUHC) - Redeemer's University<br>
...
async function sendWithResend(to: string, code: string, name: string, apiKey: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: `Redeemer's University Health Centre (RUHC) <${EMAIL_FROM}>`,
        to: [to],
        subject: "Your Verification Code - Redeemer's University Health Centre (RUHC)",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  try {
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        from: { email: EMAIL_FROM, name: "Redeemer's University Health Centre (RUHC)" },
        subject: "Your Verification Code - Redeemer's University Health Centre (RUHC)",
        content: [{
          type: 'text/html',
          value: `
