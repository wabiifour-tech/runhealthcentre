...
const smsTemplates = {
  appointment: (data: Record<string, any>) => 
    `RUHC Reminder: Dear ${data.patientName || 'Patient'}, your appointment is on ${data.date} at ${data.time} with ${data.doctor || 'Doctor'}. Arrive 15 mins early. Redeemer's University Health Centre.`,
  
  lab_result: (data: Record<string, any>) => 
    `RUHC Lab: Dear ${data.patientName || 'Patient'}, your ${data.testName || 'lab test'} results are ready. Visit the lab to collect. Redeemer's University Health Centre.`,
  
  prescription: (data: Record<string, any>) => 
    `RUHC Pharmacy: Dear ${data.patientName || 'Patient'}, your prescription is ready. Collect at the pharmacy during working hours (8AM-6PM). Redeemer's University Health Centre.`,
  
  billing: (data: Record<string, any>) => 
    `RUHC Billing: Dear ${data.patientName || 'Patient'}, your bill of ₦${data.amount?.toLocaleString() || '0'} is ready. Invoice: ${data.invoiceNo}. Visit billing for payment. Redeemer's University Health Centre.`,
  
  emergency: (data: Record<string, any>) => 
    `URGENT RUHC: ${data.message || 'Emergency notification'}. Please contact Redeemer's University Health Centre immediately. ${data.contactNumber || 'Tel: +234-XXX-XXX-XXXX'}`,
...
export async function GET(request: NextRequest) {
  ...
  return successResponse({
    service: "Redeemer's University Health Centre (RUHC) SMS Notification Service",
    status: 'active',
    mode: SMS_MODE,
    configured: {
