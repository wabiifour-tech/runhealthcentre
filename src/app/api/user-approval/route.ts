  ...
  try {
      ...
      try {
        ...
        try {
          await p.$executeRawUnsafe(`
            INSERT INTO notifications (id, "userId", type, title, message, priority, read, "createdAt")
            VALUES (
              'notif-${Date.now()}-${Math.random().toString(36).substr(2, 9)}',
              '${userId}',
              'account_status',
              '${action === 'approve' ? 'Account Approved!' : 'Account Update'}',
              '${action === 'approve' 
    ...
    try {
      ...
      try {
        await pool.query(`
          INSERT INTO notifications (id, "userId", type, title, message, priority, read, "createdAt")
          VALUES ($1, $2, 'account_status', $3, $4, $5, FALSE, $6)
        `, [
          `notif-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          userId,
          action === 'approve' ? 'Account Approved!' : 'Account Update',
