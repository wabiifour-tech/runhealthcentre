...
// API endpoints for different operations - all use /api/data
...
const STORE_TO_API_TYPE: Record<string, string> = {
  ...
  // Additional stores
  [STORES.SURGERY_BOOKINGS]: 'surgeryBooking',
  [STORES.IMMUNIZATIONS]: 'immunization',
  [STORES.BLOOD_DONORS]: 'bloodDonor',
  [STORES.AMBULANCE_CALLS]: 'ambulanceCall',
  [STORES.EQUIPMENT]: 'equipment',
  [STORES.INVENTORY]: 'inventoryItem',
  [STORES.STAFF_CERTIFICATIONS]: 'staffCertification',
  [STORES.TRAINING_RECORDS]: 'trainingRecord',
  [STORES.MEDICATION_ADMINISTRATIONS]: 'medicationAdministration',
  [STORES.INSURANCE_CLAIMS]: 'insuranceClaim',
  [STORES.PAYMENTS]: 'payment',
  [STORES.EXPENSES]: 'expense',
  [STORES.STAFF_MESSAGES]: 'staffMessage',
  [STORES.ROUTING_REQUESTS]: 'routingRequest',
}
...
async function executeSyncOperation(operation: SyncOperation): Promise<{ success: boolean; error?: string }> {
  ...
  if (!apiType) {
    return { success: false, error: `Unknown store type: ${operation.store}` }
  }
  ...
  try {
    ...
    if (operation.type === 'CREATE') {
      ...
      response = await fetch(`${API_ENDPOINT}?type=${apiType}&id=${operation.entityId}`, {
        method: 'DELETE'
      })
    } else {
...
export async function offlineFirstUpdate<T extends { id: string }>(
  ...
  try {
    const existing = await getLocal<T>(storeName, id)
    
    if (existing) {
      await saveLocal(storeName, { ...existing, ...data, id } as T)
    } else {
      // If item doesn't exist locally, create it with the provided data
      await saveLocal(storeName, { ...data, id } as T)
    }
