    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [
            new TextRun({
              text: "REDEEMER'S UNIVERSITY HEALTH CENTRE (RUHC)",
              bold: true,
              size: 36,
              color: "1E40AF"
            })
        ...
        new Paragraph({
          ...
          children: [
            new TextRun({
              text: "Redeemer's University Health Centre (RUHC) - Caring for You",
              size: 18,
              color: "1E40AF"
            })
          ]
...
export async function generateDailySummaryPDF(data: DailyReportData): Promise<void> {
  ...
  doc.text("REDEEMER'S UNIVERSITY HEALTH CENTRE (RUHC)", 105, 20, { align: 'center' })
  ...
  doc.setTextColor(30, 64, 175)
  doc.text("Redeemer's University Health Centre (RUHC) - Caring for You", 105, footerY + 38, { align: 'center' })
...
export function generateDailySummaryExcel(data: DailyReportData): void {
  ...
  const combinedCSV = `REDEEMER'S UNIVERSITY HEALTH CENTRE (RUHC) - DAILY REPORT (${data.date})
===========================================
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: "REDEEMER'S UNIVERSITY HEALTH CENTRE (RUHC) - LABORATORY REPORT", bold: true, size: 28, color: "1E40AF" })]
        }),
        new Paragraph({ children: [] }),
