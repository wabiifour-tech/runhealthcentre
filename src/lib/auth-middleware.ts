/**
 * Authentication Middleware for API Routes - PostgreSQL (Neon)
...
import { getPrisma, getPool } from './db'
...
/**
 * Verify user session from database - PostgreSQL
 * PRIMARY: Direct pg (using singleton pool), SECONDARY: Prisma, FALLBACK: Trust headers
 */
async function verifyUserSession(userId: string): Promise<{ user: AuthUser | null, dbVerified: boolean }> {
  // PRIMARY: Direct pg using singleton pool (most reliable)
  try {
    const pool = getPool()
...
export function adminOnly(
  ...
  return withAuth(handler as (request: NextRequest, user: AuthUser | null) => Promise<NextResponse>, { requireAdmin: true })
}
...
export function superAdminOnly(
  ...
  return withAuth(handler as (request: NextRequest, user: AuthUser | null) => Promise<NextResponse>, { requiredRole: 'SUPER_ADMIN' })
}
